...
class AttendanceService {
  ...
  async getEffectiveCheckInTime(employeeId, date = new Date()) {
    try {
      ...
      console.error("Error getting effective check-in time:", error);
      return "09:00";
    }
  }
  ...
  async getEffectiveCheckOutTime(employeeId, date = new Date()) {
    try {
      ...
      return companyDefault?.checkOutTime || "17:00";
    } catch (error) {
      console.error("Error getting effective check-out time:", error);
      return "17:00";
    }
  }
  ...
  async getEffectiveSchedule(employeeId, date = new Date()) {
    try {
      ...
      // ✅ Get employee without department association
      const employee = await Employee.findByPk(numericEmployeeId, {
        attributes: [
          "employeeId",
          "employeeCode",
      ...
      if (!employee) {
        throw new Error(`Employee not found with ID: ${numericEmployeeId}`);
      }

      const [employeeOverride, deptOverride, companyDefault] =
        await Promise.all([
      ...
      return {
        ...
        department: `Department ${employee.departmentId || "N/A"}`, // ✅ Show department ID instead of name
        shiftType: employee.shiftType,
        effectiveDate: formattedDate,
        checkInTime:
          employeeOverride?.checkInTime ||
          deptOverride?.checkInTime ||
          companyDefault?.checkInTime ||
          "09:00",
        checkOutTime:
          employeeOverride?.checkOutTime ||
          deptOverride?.checkOutTime ||
          companyDefault?.checkOutTime ||
          "17:00",
        lunchDuration:
          employeeOverride?.lunchDurationMinutes ||
          deptOverride?.lunchDurationMinutes ||
          companyDefault?.lunchDurationMinutes ||
          60,
        dinnerDuration:
          employeeOverride?.dinnerDurationMinutes ||
          deptOverride?.dinnerDurationMinutes ||
          companyDefault?.dinnerDurationMinutes ||
          60,
        priority: employeeOverride
          ? "Employee Override"
          : deptOverride
            ? "Department Override"
            : "Company Default",
  ...
  async issueBreakTicket(employeeId, breakType, reason = null) {
    ...
    if (!companyDefault) {
      throw new Error(
        `Company defaults not configured for ${shiftType} shift. Please contact administrator.`,
      );
    }
    ...
    if (existingTodayTicket) {
      throw new Error(
        `❌ You already took ${breakType} today. You can only take one ${breakType} break per day.`,
      );
    }
    ...
    if (existingActive) {
      throw new Error(
        `❌ You already have an active ${breakType} break. Please complete it first.`,
      );
    }
    ...
    if (breakType === "lunch") {
      ...
      if (currentTimeString < lunchStartTime) {
        throw new Error(
          `❌ Lunch break is not yet available. Lunch starts at ${lunchStartTime}.`,
        );
      }

      if (currentTimeString > latestLunchTime) {
        throw new Error(
          `❌ Lunch must start by ${latestLunchTime} (1 hour before check-out at ${checkOutTime}).`,
        );
      }
      ...
      const [currentHour, currentMinute] = currentTimeString
        .split(":")
        .map(Number);
      ...
      if (!isWithinWindow) {
        throw new Error(
          `❌ Dinner break is only available from ${dinnerStartTime} until ${checkOutTime} (next day).`,
        );
      }
    }
    ...
    const employeeOverride = await EmployeeOverride.findOne({
      where: {
        ...
        [Op.or]: [{ effectiveTo: { [Op.gte]: now } }, { effectiveTo: null }],
      },
      order: [["effectiveFrom", "DESC"]],
    });
    ...
    if (durationMinutes == null) {
      if (employee && employee.departmentId) {
        const deptShiftType = (
          employee.shiftType || (breakType === "lunch" ? "day" : "night")
        ).toLowerCase();
        const deptOverride = await DepartmentOverride.findOne({
          where: {
            departmentId: employee.departmentId,
    ...
    if (durationMinutes == null) {
      ...
      if (durationMinutes == null) {
        throw new Error(
          `No ${breakType} duration configured. Please set up company defaults first.`,
        );
      }
    }
  ...
  async checkExpiredBreaks() {
    ...
    for (const ticket of expiredBreaks) {
      ...
      const lateMinutes = Math.floor(
        (new Date() - ticket.expectedReturnTime) / 60000,
      );
      let status = "late";
  ...
  async recordAttendance(
    ...
    const effectiveCheckIn = await this.getEffectiveCheckInTime(
      employeeId,
      date,
    );
    const effectiveCheckOut = await this.getEffectiveCheckOutTime(
      employeeId,
      date,
    );
  ...
  async calculateOvertime(employeeId, date) {
    ...
    const overtimeAfter = await this.getEffectiveOvertimeAfter(
      employeeId,
      date,
    );
    ...
    if (attendance.checkOutTime > overtimeThreshold) {
      overtimeMinutes = Math.floor(
        (attendance.checkOutTime - overtimeThreshold) / 60000,
      );
    }
    ...
    const overtimeRate = await OvertimeRate.findOne({
      where: {
        ...
        [Op.or]: [{ effectiveTo: { [Op.gte]: date } }, { effectiveTo: null }],
      },
      order: [["effectiveFrom", "DESC"]],
    });
  ...
  async getHolidayOvertimeRate(date) {
    try {
      ...
      console.error("Error getting holiday overtime rate:", error);
      return 2.5;
    }
  }
