...
exports.processRequests = async (req, res) => {
  ...
  try {
    ...
    for (const request of requests) {
      ...
      // ================================================================
      // 8c. DETERMINE ACTION (Stock In or Stock Out)
      // ================================================================
      ...
      for (const item of request.items) {
        try {
          ...
          // Check if balance exists for this store, group, and item
          ...
          // Now process the stock movement
      ...
      // ================================================================
      const storeWithGroups = await Store.findByPk(parseInt(storeId), {
        include: [
          {
            model: Group,
            as: 'groups',
...
exports.processRequestForGroup = async (req, res) => {
  ...
  try {
    ...
    for (const item of request.items) {
      try {
        // Find the balance for this store, group, and item
    ...
    const responseData = {
      ...
      errors: errors,
      isFullyProcessed: allProcessed && allGroupIds.length > 0,
  ...
  try {
    ...
    const result = requests.map(request => {
      ...
      return {
        ...
        // This will be done in the frontend or with additional query
      };
    });
