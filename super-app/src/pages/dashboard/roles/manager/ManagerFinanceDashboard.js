// super-app/src/pages/dashboard/roles/manager/ManagerFinanceDashboard.js
...
let mobileFinanceDashboardService = null;
try {
  // eslint-disable-next-line global-require
  mobileFinanceDashboardService =
    require('../../../../stores/mobileFinanceDashboardService').default ||
    require('../../../../stores/mobileFinanceDashboardService').mobileFinanceDashboardService;
} catch (e) {
  mobileFinanceDashboardService = null;
}
...
export default function ManagerFinanceDashboard({
  ...
  const [financeData, setFinanceData] = useState(EMPTY_DATA);
  ...
  const loadSummary = useCallback(async ({ silent = false } = {}) => {
    try {
      ...
      if (!mobileFinanceDashboardService?.getFinanceSummary) {
        setFinanceData(EMPTY_DATA);
        return;
      }

      const res = await mobileFinanceDashboardService.getFinanceSummary();

      if (res?.success) {
        setFinanceData({
          pendingApprovals: Number(res.data?.pendingApprovals ?? 0),
          paidThisMonth: Number(res.data?.paidThisMonth ?? 0),
          budgetUsed: Number(res.data?.budgetUsed ?? 0),
          budgetTotal: Number(res.data?.budgetTotal ?? 0),
          pendingExpenses: Number(res.data?.pendingExpenses ?? 0),
        });
      } else {
        setError(res?.error || 'Failed to load finance summary');
      }
    } catch (e) {
  ...
  const goTo = (destination) => {
    ...
    console.warn('⚠️ No navigation callback for finance destination:', destination);
  };

  const budgetRemaining = Math.max(
    0,
    financeData.budgetTotal - financeData.budgetUsed
  );
  ...
  const sections = [
    {
      ...
      title: 'Budgets',
      emoji: '🎯',
      subtitle: 'Allocations and utilization',
      accent: '#F59E0B',
      softBg: '#FFFBEB',
      softBgDark: '#78350F',
      destination: 'budgets',
      stats: [
        { label: 'Used', value: financeData.budgetUsed, color: '#F59E0B' },
        { label: 'Left', value: budgetRemaining, color: '#10B981' },
        { label: 'Total', value: financeData.budgetTotal, color: '#F59E0B' },
      ],
    },
    {
      key: 'expenses',
      title: 'Expenses',
      emoji: '🧾',
      subtitle: 'Claims awaiting approval or payment',
      accent: '#EF4444',
      softBg: '#FEF2F2',
      softBgDark: '#7F1D1D',
      ...
      stats: [
        {
          label: 'Pending',
          value: financeData.pendingExpenses,
          color: '#EF4444',
        },
        {
          label: 'Paid',
          value: financeData.paidThisMonth,
          color: '#10B981',
        },
        {
          label: 'Total',
          value: financeData.pendingExpenses + financeData.paidThisMonth,
          color: '#EF4444',
        },
      ],
    },
    {
      ...
      title: 'Approvals',
      emoji: '✅',
      subtitle: 'Finance sign-offs in the queue',
      accent: '#3B82F6',
      softBg: '#EFF6FF',
      softBgDark: '#1E3A8A',
      ...
      stats: [
        {
          label: 'Pending',
          value: financeData.pendingApprovals,
          color: '#3B82F6',
        },
    ...
    },
    {
      ...
      destination: 'reports',
      stats: [
        { label: 'Monthly', value: 0, color: '#8B5CF6' },
        { label: 'Quarter', value: 0, color: '#8B5CF6' },
        { label: 'Annual', value: 0, color: '#8B5CF6' },
      ],
    },
  ];
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Track budgets, expenses and approvals
        </Text>
      </View>
