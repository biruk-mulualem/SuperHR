...
/**
 * Send submitted prices to the boss.
 * Creates a purchase notification for the manager/admin.
 */
async sendToBoss(requestId: number, message: string | null = null) {
  try {
    const response = await api.post(
      `/purchase-follow-ups/${requestId}/send-to-boss`,
      { message }
    );
    return {
    ...
    return {
      success: false,
      error: error.response?.data?.error || 'Failed to send to boss',
    };
  }
}
