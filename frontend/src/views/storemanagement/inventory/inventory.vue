...
const downloadTemplate = () => {
  const headers = [
    'name', 'standardName', 'description', 'brand', 'model', 'barcode',
    'categoryName', 'uomCode', 'conversionUomCode', 'conversionValue', 'costPrice', 'specText'
  ];
...
const exportSelectedReport = async () => {
  ...
  try {
    ...
    const params = {
      categoryId: categoryId,
      status: filterStatus.value || undefined,
      format: exportFormat.value || 'xlsx',
    };
...
const openImportModal = () => {
  ...
  importProgress.value = { total: 0, processed: 0, success: 0, failed: 0, remaining: 0, percentage: 0 };
};
...
@media (max-width: 900px) {
  ...
  .item-form .form-row { flex-direction: column; gap: 8px; }
}
