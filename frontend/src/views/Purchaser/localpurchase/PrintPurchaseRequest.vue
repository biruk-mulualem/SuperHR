...
const remarkGroups = computed(() => {
  ...
  items.forEach((item, index) => {
    ...
    if (currentRemark === null) {
      ...
      groups.push({ remark: currentRemark, startIndex, rowspan: count });
      currentRemark = remark;
      startIndex = index;
      count = 1;
    }
...
const loadRequest = async (requestId: string) => {
  ...
  try {
    ...
    if (!response.success || !response.data) {
      console.warn("❌ Purchase request not found:", response.error);
      requestData.value = null;
      return;
    }

    // Normalize the API response into the shape this template expects
    const raw: any = response.data;
    requestData.value = {
      id: raw.id,
      prNumber: raw.prNumber,
      department: raw.department,
      expertName: raw.expertName,
      ...
      items: (raw.items || []).map((it: any) => ({
        id: it.id,
        code: it.code,
        // `name` is resolved by the backend from the master `items` table
        name: it.name || it.code || "Unknown Item",
        brand: it.brand || "",
        model: it.model || "",
        uom: it.uom || it.baseUom || "",
      ...
      approvedDocFront: raw.approvedDocFront
        ? resolveDocUrl(raw.approvedDocFront)
        : undefined,
      approvedDocFrontName: raw.approvedDocFrontName,
      approvedDocBack: raw.approvedDocBack
        ? resolveDocUrl(raw.approvedDocBack)
        : undefined,
      approvedDocBackName: raw.approvedDocBackName,
    };
