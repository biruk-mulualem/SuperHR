    ...
    <div class="card-header">
      <div class="header-left">
        ...
        <div class="header-info">
          <h3>Today's Lunch Tracking</h3>
          <p class="header-subtitle">Track employee lunch breaks</p>
        </div>
...
const fetchData = async () => {
  try {
    ...
    const response = await attendanceService.getLunchHistory(params)
    ...
    console.error('Failed to load lunch tickets:', err)
    error.value = err.response?.data?.error || 'Failed to load lunch tickets'
  }
}
...
const issueTicket = async () => {
  ...
  try {
    await attendanceService.issueBreakTicket(selectedEmployeeId.value, 'lunch')
    await fetchData()
    closeIssueModal()
    showToast('Lunch ticket issued successfully', 'success')
  } catch (err) {
