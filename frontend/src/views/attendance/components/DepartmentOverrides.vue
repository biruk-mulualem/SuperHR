  ...
  <div class="config-card">
    <div class="card-header">
      <div class="header-left">
        ...
        <div class="header-info">
          ...
          <p class="header-subtitle">Override company defaults for specific departments</p>
        </div>
        <span class="badge">{{ overrides.length }}</span>
      </div>
      <div class="header-actions">
        <button class="btn-add-small" @click="openAddModal">
          <span class="btn-icon">+</span> Add Department
        </button>
    ...
    <div class="card-body">
      <div v-if="loading" class="loading-state">
        ...
        <p>Loading department overrides...</p>
      </div>
      ...
      <div v-else class="table-container">
        <table class="data-table">
          <thead>
            <tr>
              <th>Department</th>
              <th>Check-In <span class="th-sub">Time</span></th>
              <th>Check-Out <span class="th-sub">Time</span></th>
              <th>Lunch <span class="th-sub">(minutes)</span></th>
              <th>OT After <span class="th-sub">Time</span></th>
              <th style="width: 100px">Actions</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="dept in overrides" :key="dept.id">
              <td class="department-cell">
                <span class="dept-name">{{ dept.departmentName }}</span>
              </td>
              <td>
                <input type="time" v-model="dept.checkInTime" class="input-sm">
              </td>
              <td>
                <input type="time" v-model="dept.checkOutTime" class="input-sm">
              </td>
              <td>
                <input type="number" v-model="dept.lunchDurationMinutes" class="input-sm" min="0" step="5">
              </td>
              <td>
                <input type="time" v-model="dept.overtimeAfterTime" class="input-sm">
              </td>
              <td class="action-buttons">
                <button class="btn-icon edit" @click="openEditModal(dept)" title="Edit department override">
                  ✏️
                </button>
                <button class="btn-icon delete" @click="confirmDelete(dept.id, dept.departmentName)" title="Delete department override">
                  🗑️
                </button>
              </td>
            </tr>
            <tr v-if="overrides.length === 0">
              <td colspan="6" class="empty-row">
                <div class="empty-state-small">
                  ...
                  <p>No department overrides configured</p>
      ...
      <div class="modal" @click.stop>
        <div class="modal-header">
          <div class="modal-header-left">
            ...
            <h3>{{ editingItem ? 'Edit' : 'Add' }} Department Override</h3>
          </div>
        ...
        <div class="modal-body">
          <div class="form-field">
            ...
            <div class="select-wrapper">
              <select v-model="formData.departmentId" class="input" :disabled="editingItem">
...
const fetchDepartments = async () => {
  try {
    ...
    if (result.success && result.data) departments.value = result.data
  } catch (err) {
